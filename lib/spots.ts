...
export type Spot = {
  ...
  /**
   * 「近くのスポット」に表示する slug を運営者が手動指定する上書き。
   * 指定すると自動算出（同駅/同区）より優先し、この順番で表示する。
   * admin（/admin/spots/edit）から施設ごとに選択できる。
   */
  nearbySlugs?: string[];
  /**
   ...
   *
   * until（YYYY-MM-DD）を過ぎたら描画しない。恒久の本文に「◯月◯日まで停止」と
   * 書き足すと、期間が終わったあとに誰も消さず古い注意書きが residue として残る。
   * 期限を持たせて自動で消えるようにするのが、確認日と同じ考え方。
   */
  notice?: {
    /** 表示文。いつからいつまでかを文中にも書く */
    text: string;
    /** この日を過ぎたら表示しない（YYYY-MM-DD） */
    until: string;
    /** 出どころ（施設公式など） */
    source?: string;
  };
...
export const SPOTS: Partial<Record<AreaSlug, Spot[]>> = {
  ...
  saitama: [
    ...
    },
    {
      ...
      pricing: { preschool: 'キッズメニューのある店あり' },
      strollerAccess: true,
      facilities: { diaperChange: 'yes', nursingRoom: 'yes', strollerRental: 'yes' },
    },
    {
  ...
  osaka: [
    ...
    // ===== 大阪 子連れOKレストラン（梅田/難波/天王寺の大型商業施設。実在施設のみ） =====
    {
      name: 'グランフロント大阪（梅田）', category: 'restaurant', place: 'indoor', ages: ['0-1', '2-3', '4-6'],
      ward: '梅田', city: '大阪市北区', budget: 'mid',
      note: 'JR大阪駅直結の複合施設。ベビーカーで回れ、子連れOK・キッズメニューのある飲食店が多数。館内にベビー休憩室・授乳室・おむつ替えあり',
      nearestStation: 'osaka-umeda', walkMinutes: 3,
      pricing: { adult: 'ランチ1,200円〜', preschool: 'キッズメニューのある店あり' },
      strollerAccess: true, babyChair: true, kidsMenu: true,
      facilities: { diaperChange: 'yes', nursingRoom: 'yes', strollerRental: 'yes' },
    },
    {
      name: 'ルクア大阪（梅田）', category: 'restaurant', place: 'indoor', ages: ['0-1', '2-3', '4-6'],
      ward: '梅田', city: '大阪市北区', budget: 'mid',
      note: 'JR大阪駅直結の駅ビル。バルチカ等の飲食フロアに子連れOKの店が入る。館内にベビー休憩室・おむつ替えあり',
      nearestStation: 'osaka-umeda', walkMinutes: 1,
      pricing: { adult: 'ランチ1,200円〜', preschool: 'キッズメニューのある店あり' },
      strollerAccess: true, babyChair: true, kidsMenu: true,
      facilities: { diaperChange: 'yes', nursingRoom: 'yes', strollerRental: 'yes' },
    },
    {
      name: 'なんばパークス（難波）', category: 'restaurant', place: 'indoor', ages: ['0-1', '2-3', '4-6'],
      ward: '難波', city: '大阪市浪速区', budget: 'mid',
      note: '難波駅直結の複合施設。屋上庭園あり、ベビーカーで回れ、子連れOK・キッズメニューのある飲食店が多数。授乳室・おむつ替えあり',
      nearestStation: 'osaka-namba', walkMinutes: 3,
      pricing: { adult: 'ランチ1,200円〜', preschool: 'キッズメニューのある店あり' },
      strollerAccess: true, babyChair: true, kidsMenu: true,
      facilities: { diaperChange: 'yes', nursingRoom: 'yes', strollerRental: 'yes' },
    },
    {
      name: 'あべのキューズモール（天王寺・阿倍野）', category: 'restaurant', place: 'indoor', ages: ['0-1', '2-3', '4-6'],
      ward: '天王寺・阿倍野', city: '大阪市阿倍野区', budget: 'low',
      note: '天王寺駅徒歩の大型商業施設。子連れ向けの飲食店が多く、ベビーカー貸出・授乳室・おむつ替えが充実',
      nearestStation: 'osaka-tennoji', walkMinutes: 3,
      pricing: { adult: 'ランチ1,000円〜', preschool: 'キッズメニュー多数' },
      strollerAccess: true, babyChair: true, kidsMenu: true,
      facilities: { diaperChange: 'yes', nursingRoom: 'yes', strollerRental: 'yes', kidsSpace: 'yes' },
    },
    {
      name: 'あべのハルカス近鉄本店（天王寺・阿倍野）', category: 'restaurant', place: 'indoor', ages: ['0-1', '2-3', '4-6'],
      ward: '天王寺・阿倍野', city: '大阪市阿倍野区', budget: 'mid',
      note: '天王寺駅直結のデパート。レストラン街は子連れOKの店が入り、館内にベビー休憩室・授乳室・おむつ替えあり',
      nearestStation: 'osaka-tennoji', walkMinutes: 1,
      pricing: { preschool: 'キッズメニューのある店あり' },
      strollerAccess: true,
      facilities: { diaperChange: 'yes', nursingRoom: 'yes', strollerRental: 'yes' },
    },
...
export const TOKYO_RESTAURANTS: Spot[] = [
  ...
  {
    // 流入は「矢場とん キッズメニュー / 子供メニュー」がほぼ全て（GSC 28日で97クリック・平均5.2位）。
    // 2026-08-12 に公式サイト（yabaton.com）のおしながき・店舗案内・よくあるご質問を確認したところ、
    // お子様メニュー／キッズメニューの区分も、ベビーカー・ベビーチェアの記載も無かった。
    // 以前は strollerAccess/babyChair を true にしていたが根拠が無いので落とし、
    // 「お子様メニューは無い」という事実を直答として note と faq に置く。
    name: '矢場とん', category: 'restaurant', place: 'indoor', ages: ['2-3', '4-6'],
    city: '名古屋市ほか',
    note: '公式のおしながきにお子様メニュー（キッズメニュー）の設定はなし。子どもと食べるなら味噌だれを別皿にしてもらい、ひれとんかつ（中・120g）やみそかつ丼を取り分けるのが現実的です。',
    officialUrl: 'https://www.yabaton.com/',
    pricing: { adult: '1,500-2,500円', elementary: '取り分け可', preschool: '取り分け可' },
    hiddenTip: '味噌ダレ別皿に頼めば子どもはソースなしで食べられる',
    faq: [
      {
        q: '矢場とんにキッズメニュー（お子様メニュー）はありますか？',
        a: 'ありません。公式サイトのおしながきは「名物みそかつ」「フライ・コロッケ」「ご飯もの」「サラダ」「おつまみ・単品」「お飲物」の6区分で、お子様メニュー・キッズメニューの区分は設けられていません（2026年8月時点）。お子さんの分は大人のメニューからの取り分けが前提になります。',
      },
      {
        q: '子どもには何を頼めばいいですか？',
        a: 'とんかつなら小さめの「ひれとんかつ（中・120g）」、ご飯ものなら「みそかつ丼」やカレー・ハヤシライスが取り分けやすい選択です。味噌だれは別皿にしてもらえるので、お子さんにはたれをかけずに出せます。',
      },
      {
        q: 'ベビーカーで行っても大丈夫ですか？',
        a: '公式サイトにはベビーカー・ベビーチェアの可否に関する記載がありません（2026年8月時点）。矢場とんは百貨店・駅ビル・アウトレット内の店舗から路面の本店まで立地がさまざまで、フロア構成も店舗ごとに違います。来店前に行く店舗へ直接ご確認ください。',
      },
    ],
      summerCool: true,
  },
  ...
  // ===== 東京（25店舗） =====
  {
    ...
    pricing: { elementary: 'キッズメニュー600円〜', preschool: '600円〜', infant: '離乳食持ち込みOK' },
    strollerAccess: true, babyFood: true,
  ...
  },
  {
    ...
    pricing: { elementary: 'キッズメニュー店舗あり', preschool: '取り分け可' },
    strollerAccess: true,
  ...
  },
  {
    ...
    pricing: { elementary: 'キッズメニュー600円〜', preschool: '取り分け可' },
    strollerAccess: true,
  ...
  },
  {
    ...
    pricing: { elementary: '取り分け可', preschool: '取り分け可' },
    strollerAccess: true,
  ...
  },
  {
    ...
    pricing: { elementary: '取り分け可', preschool: '取り分け可' },
    strollerAccess: true, privateRoom: true,
  ...
  },
  {
    ...
    pricing: { elementary: 'キッズメニュー店舗による', preschool: '取り分け可' },
    strollerAccess: true,
  ...
  },
  {
    ...
    pricing: { elementary: 'キッズメニューあり店舗', preschool: '取り分け可' },
    strollerAccess: true,
