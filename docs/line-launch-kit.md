...
### 友だち追加のオファー＝子連れ外食マップ（2026-07-28にサイト側へ反映）

サイトのCTA（`components/common/LineCta.tsx`）は
**「【全部行った】ベビーカーで入れる東京の子連れ外食50選」マップのプレゼント**で友だち追加を取る。

- 実体: Googleマイマップ（mid `1CFmg4jFSisLtRzVHMG7lFDFxxdZIb1I`）。運営者が0〜2歳の子を連れて実際に回った東京50店。
- 各ピンに エリア・最寄駅／ジャンル／ベビーカー（◎席まで横付け=29店・○店内OK=21店）／座敷・個室／また行く度★／本音の説明／⚠️行く前の注意／食べログURL。
- **このURLはサイト上に置かない。**サイトで配ると友だち追加する理由が消える。渡すのはLINEのあいさつメッセージだけ。
- CTAに書いている数字（50店・29店・20ジャンル）はマップの実データ由来（2026-07-28にKMLで実測）。
  **マップにピンを足したらCTA内の定数 `SHOP_COUNT` / `STROLLER_SIDE_BY_SIDE` も更新すること。**

### あいさつメッセージ（友だち追加直後に自動送信・通数カウント外）

```
友だち追加ありがとうございます🌱
お約束の「子連れ外食マップ」はこちらです。

▼【全部行った】ベビーカーで入れる東京の子連れ外食50選
https://www.google.com/maps/d/viewer?mid=1CFmg4jFSisLtRzVHMG7lFDFxxdZIb1I

0〜2歳の子を連れて実際に回った50店です。
各店に「ベビーカーは席まで横付けできるか」
「座敷・個室」「また行く度」「行く前の注意」を
書いてあります。現在地から近い順に探せます📍
...
**サイトのCTAは「その場でマップのリンクが届きます」と書いている。あいさつメッセージにマップURLが入っているかをデプロイ前に必ず確認すること。**
...
- メモ: 配布した外食マップを後から見返したい人が必ず出るので、画像を作り直すタイミングで
  1セルを `🍽 子連れ外食マップ → https://www.google.com/maps/d/viewer?mid=1CFmg4jFSisLtRzVHMG7lFDFxxdZIb1I`
  に差し替えるとよい（現行画像のままでも、あいさつメッセージのトーク履歴を遡れば辿れる）。
...
## 2-1. 基本リズム: 金曜20:00「週末どこ行く？」固定

- **週1回・金曜20:00** に固定。寝かしつけ後にスマホを見る時間帯で、週末の予定を考え始めるタイミング
- 構成は毎週同じ「**天気 × 3択**」フォーマット:
  1. 導入1行（今週末の天気をひとこと）
  2. ☀️晴れたら → 外あそび系のリンク
  3. ☔雨なら → 屋内・家あそび系のリンク
  4. 🥵猛暑なら／👜持ち物 → 季節系のリンク
- 予約配信を使う: 管理画面「メッセージ配信 > 作成」→ 配信日時を金曜20:00に指定。**週の好きなタイミングに書いて予約**しておけば金曜に手が空く
- フォーマット固定のメリット: 制作10分以内で回せる／読者が「金曜のあれ」と認識して開封が安定する

> **2026-08-16 変更**: 配信時刻を **17:00 → 20:00 に統一**した。
> 理由: `vercel.json` の cron `/api/cron/line-weekend` が金曜20:00 JST（`0 11 * * 5`）に自動broadcastしており、
> 手書き原稿を17:00に予約配信すると**同じ金曜に2通**届いて無料枠200通/月を倍速で消費していた。
> 対応として**cronの自動配信を停止**（vercel.jsonから当該行を削除）し、**手書き原稿の予約配信に一本化**、
> 時刻は実配信に合わせて20:00に揃えた。サイト側の表記（`components/common/LineCta.tsx`）も20時に修正済み。
> ⚠️ **LINE公式アカウント側の「あいさつメッセージ」は管理画面にしか無いので、上の §1 の文面に合わせて手動で更新すること**（リポジトリを直しても本番の挨拶文は変わらない）。
> なお `/api/cron/line-weekend` のコードは残してある（`?preview=1` で本人宛のテスト送信に使える）。再びcron化する場合は二重送信に注意。
...
- 登録率を上げる打ち手: 記事末尾に「金曜20時に週末プランが届きます」と具体的なベネフィットを書く／雨の日記事にだけ出すなど文脈マッチ
